# First-Run Composer Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the passive ActivationPanel with an interactive first-run composer that helps new users create their first LinkedIn post in under 60 seconds.

**Architecture:** One new API route (`/api/infer-context`) for AI inference. One new component (`FirstRunComposer`) with a 6-state machine. Dashboard swaps `ActivationPanel` for `FirstRunComposer` when `has_generated=false`. Generation uses existing `/api/runs` with `social_posts` workflow.

**Tech Stack:** Next.js App Router, React 18, Supabase client, Anthropic SDK (via existing `callClaude`), Tailwind CSS, existing design tokens.

---

### Task 1: Create `/api/infer-context` Route

**Files:**
- Create: `app/api/infer-context/route.ts`
  ...
  try {
    ...
    if (!description) {
      return NextResponse.json(
        { audience: "", offer: "", outcome: "", confidence: "low" as const },
      );
    }
...
```bash
git add app/api/infer-context/route.ts
git commit -m "feat(activation): add /api/infer-context route for first-run composer"
```

---

### Task 2: Create `FirstRunComposer` Component — Input & Inferring States

**Files:**
- Create: `components/ui/first-run-composer.tsx`

- [ ] **Step 1: Create the component with input and inferring states**
...
export function FirstRunComposer({ workspaceId }: FirstRunComposerProps) {
  ...
  async function saveContextAndGenerate(context: InferredContext) {
    // 1. Read existing brand_notes to append
    let existingNotes = "";
    try {
...
```bash
git add components/ui/first-run-composer.tsx
git commit -m "feat(activation): add FirstRunComposer component with 6-state flow"
```

---

### Task 3: Integrate FirstRunComposer into Dashboard

**Files:**
- Modify: `app/app/dashboard/page.tsx`

- [ ] **Step 1: Add workspaceId state and populate it from bootstrap response**

In the state declarations block (around line 142), add:

```typescript
const [workspaceId, setWorkspaceId] = useState<string | null>(null);
```

In the `useEffect` after line 171 (`const { is_new, workspace_id } = await bootstrapRes.json();`), after the existing `if (!workspace_id)` guard (around line 193), add:

```typescript
setWorkspaceId(workspace_id);
```

- [ ] **Step 2: Replace ActivationPanel import with FirstRunComposer**
...
- [ ] **Step 3: Replace ActivationPanel render with FirstRunComposer**

Find the activation panel section (around line 507-512):
...
Note: removed `mb-10` from wrapper since `FirstRunComposer` manages its own margin.
...
git commit -m "feat(activation): integrate FirstRunComposer into dashboard, replace ActivationPanel"
```

---

### Task 4: Type-Check & Manual Smoke Test
...
- [ ] **Step 2: Verify the /api/infer-context route works**

Test with curl (requires a running dev server with auth, or test directly):
...
```bash
npx tsc --noEmit --pretty 2>&1 | grep -i "cannot find"
```
Expected: No output (no missing imports)
...
#   app/api/infer-context/route.ts (new)
#   components/ui/first-run-composer.tsx (new)
