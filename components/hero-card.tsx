...
const heroItems = [
  ...
  {
    key: "sopGenerator" as const,
    title: "Draft an SOP",
    subtitle:
      "Turn process knowledge into a clear operating asset.",
    href: "/app/operations",
    color: accentOrange,
...
export function HeroCard({ completion }: HeroCardProps) {
  const completedCount = completion
    ? [completion.socialPosts, completion.coldEmail, completion.sopGenerator].filter(Boolean).length
    : 0;
